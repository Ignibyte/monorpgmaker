...
| Visual **map paint** editor (Avalonia — `MonoRpgMaker.Studio`) | 🟡 | first cut (#16): Tiled-like LPC sprite painting + New/Save/Load `$data`; **#19 began the Studio-v2 arc** (added event placement — see below). One fixed tileset, plain index paint; the tileset picker / autotiles / layers / multi-map are the remaining v2 slices. Avalonia locked (D-0022) |
