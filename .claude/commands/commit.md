...
Read [CONSTITUTION.md](../../CONSTITUTION.md) §0 (no baselines, source-fix only) and §15 (transcript is truth). The `enforce-commit-gate.sh` hook blocks this commit unless `bin/gate.sh` printed `GATE GREEN [full]` after your last code change — so step 1 is mandatory, not advisory.
...
1. **Run the full gate and paste the real output:**
   ...
   bin/gate.sh
   ```
   It must end `GATE GREEN [full]`. If any step is red, STOP and fix at the source — do not stage, do not weaken a gate, do not lower a coverage/mutation floor. Re-run until green.
...
3. **Stage + review** — `git add -A` then show `git diff --cached --stat`; sanity-check nothing unintended (no `.mcp.json`, no secrets, no `bin/`/`obj/` build output) is staged.
