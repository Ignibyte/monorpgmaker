...
## Steps
1. **Parse the request** (`$ARGUMENTS`) — determine intent and the systems involved (map/tilemap, entities/actors, database, events, editor, runtime/player, rendering, input, save/load…).
...
- **Open the run's AAR:** `aar-open` with `intent` + the `ticket_id` you just minted. Record the returned `aar_id` in the notes — `inspect` (`failure-record`) and `complete` (`aar-submit`) capture into it.
