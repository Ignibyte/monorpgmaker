...
## Steps
1. **Update project docs** if behavior/architecture changed — the relevant `docs/*.md`, `CLAUDE.md` if conventions shifted. Keep `docs/decisions.md` current if a decision was made/changed.
...
3. **Close the forge ticket** — `ticket-close` (or `ticket-update` to a terminal status). Move the local ticket doc from `docs/planning/tickets/open/` to `closed/`.
