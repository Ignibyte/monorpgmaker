...
# Only application code is gated. Docs, .claude/, planning, project/config files
# are exempt. (In `case`, `*` matches `/`, so src/*.cs spans nested dirs.)
case "$NORMALIZED" in
    docs/*|.claude/*|bin/*|*.md|*.json|*.toml|*.yaml|*.yml|*.props|*.targets|*.sln|*.slnx|*.csproj|*.config|*.env*|*.html|*.css|*.svg|*.mgcb) exit 0 ;;
esac
case "$NORMALIZED" in
    src/*.cs|tests/*.cs) : ;;  # gated
    *) exit 0 ;;
esac
...
# Real UUID required (not a template placeholder). BSD-grep compatible.
