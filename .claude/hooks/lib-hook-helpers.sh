#!/usr/bin/env bash
# =============================================================================
# lib-hook-helpers.sh — Shared helpers for monorpgmaker enforcement hooks
# =============================================================================
#
...
# HEAD + the worktree content of every tracked-or-untracked .cs file under
# src/ tests/. bin/gate.sh writes this to .git/monorpgmaker-gate-receipt on a
# FULL green; enforce-commit-gate.sh recomputes it at `git commit` and allows
# the commit only if they match — so a green binds to the EXACT worktree it ran
# on. The verdict can't be forged by printing/echoing "GATE GREEN" or by reading
# a file that contains it (no receipt is written), and any post-green edit by ANY
# tool (Write, Edit, or a Bash heredoc) invalidates it (the content hash changes).
# Hashing the union of tracked+untracked file CONTENT (not `git diff`) makes it
# invariant to `git add` staging, so the normal gate -> add -> commit flow stays
# green. Bash 3.2 + BSD-safe (no \K/\s/-P; shasum, not sha256sum).
gate_state_hash() {
    local root="${PROJECT_ROOT:-$(pwd)}" f
    {
        git -C "$root" rev-parse HEAD 2>/dev/null || printf 'no-head\n'
        {
            git -C "$root" ls-files -- src tests 2>/dev/null
            git -C "$root" ls-files --others --exclude-standard -- src tests 2>/dev/null
        } | LC_ALL=C sort -u | while IFS= read -r f; do
            case "$f" in
                *.cs) [ -f "$root/$f" ] && shasum "$root/$f" 2>/dev/null ;;
            esac
        done
...
# The pipeline command alphabet (used by the two detectors below).
