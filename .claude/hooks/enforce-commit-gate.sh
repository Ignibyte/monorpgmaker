...
# — a `git commit` that includes C# source is BLOCKED unless bin/gate.sh left a
# RECEIPT (.git/monorpgmaker-gate-receipt) proving a FULL green ran on the EXACT
# current worktree.
#
# The receipt is a CONTENT FINGERPRINT (gate_state_hash), not a transcript
# string. That closes the holes a string-match has:
...
# Detect a real `git ... commit` in any common spelling.
